...
|---|---|---|
| کلید خصوصی | فقط داخل AGW (کیف پول هوشمند) | اپ هرگز به آن دسترسی ندارد |
...
### لایهٔ کیف پول — Abstract Global Wallet (AGW)

ورود و پرداخت فقط از مسیر رسمی `@abstract-foundation/agw-react` انجام می‌شود:

- **ورود**: `useLoginWithAbstract().login()` — مودال رسمی AGW (ایمیل / QR / اکانت اجتماعی). هیچ افزونه‌ای لازم نیست؛ نشست در iframe امن Privyِ خود AGW نگهداری می‌شود و بین اپ‌های Abstract مشترک است.
- **هویت**: `useAccount()` (wagmi که خود AGW پیکربندی می‌کند) — آدرس، حالت اتصال، رویدادهای قطع اتصال.
- **پرداخت**: `useAbstractClient()` → `abstractClient.sendTransaction({ to: token, data })`. امضای EIP-712 داخل کیف پول هوشمند انجام می‌شود و گس تراکنش‌ها توسط Paymaster شبکهٔ Abstract اسپانسر می‌شود (کاربر فقط PENGU می‌پردازد).
- **زنجیره**: تعریف chain مستقیماً `abstract` از `viem/chains` است تا هیچ انحرافی از پارامترهای رسمی شبکه ممکن نباشد؛ متادیتای نمایشی (لینک Abscan و…) در `config.ts`.
- **حذف‌شده‌ها**: هرگونه کد EIP-6963/window.ethereum/سوییچ شبکه — AGW ذاتاً روی Abstract است و «شبکهٔ اشتباه» معنا ندارد.
